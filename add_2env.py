...
def set_env(desc="Username", always_upper=True, sensitive=False, debug=True):
    ...
    if always_upper:
        print(f"**** Variable NAME will be set to all uppercase per convention...")
    env_var_name = input(f"\nPlease enter name of the {desc} environment variable: ")

    if sensitive:
        env_var_value = getpass.getpass(
            f"Please enter {desc} sensitive environment variable value "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script Description", epilog="Usage: ' python add_2env.py' "
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        default=False,
        help="When True, script will print out " "all current environment variables",
    )
